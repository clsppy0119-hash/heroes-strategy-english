...
describe('marchDurationMs', () => {
  it('主城旁邊最快', () => {
    expect(marchDurationMs(CITY_X + 1, CITY_Y)).toBe(MARCH_BASE_MS + MARCH_MS_PER_STEP);
  });
  ...
  it('越遠越久', () => {
    ...
    for (const tile of [...createMap()].sort(
      ...
      const duration = marchDurationMs(tile.x, tile.y);
  ...
  it('最遠的角落也在一分鐘以內——等待不是內容', () => {
    const longest = Math.max(...createMap().map((tile) => marchDurationMs(tile.x, tile.y)));
    expect(longest).toBeLessThan(60_000);
  });
});

describe('抵達', () => {
  const march = startMarch('3,2', 3, 2, T0);

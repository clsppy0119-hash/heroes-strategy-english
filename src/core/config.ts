...
/**
 * 行軍時間＝這個底數加上「離主城幾步 × 每步」。
 *
 * 主城旁邊 9 秒，最遠的角落 29 秒。刻意短：v0.2 真正要玩家等的是糧草
 * （以小時計），行軍只是把「下令」跟「開打」拉開，讓時間在單次遊玩裡
 * 也看得見。等待本身不是內容，v0.1 的實測已經說過玩家「一下子就不想玩了」，
 * 在那上面再加分鐘級的空轉是最快讓人關掉分頁的做法。
 */
export const MARCH_BASE_MS = 5_000;
export const MARCH_MS_PER_STEP = 4_000;

/**
 * 士氣。
 *
 * 隊伍在外面待越久、走越遠，打得越沒力；回到主城就補滿。這是「回城」
 * 唯一的實質理由——在這之前回城只換位置，換不到任何東西。
 *
 * ## 為什麼有下限
 *
 * 沒有下限的話，遠征夠久之後連全部答對都打不穿守軍，玩家會卡在一場
 * 數學上贏不了的仗裡。那正是 v0.1 實測找出來的根因（#14），不能再犯一次。
 * derive.ts 在載入時會驗證「最低士氣下全對仍然打得贏」，不成立就丟錯。
 */
export const MORALE_FULL = 1;
export const MORALE_FLOOR = 0.75;

/** 每走一格掉多少士氣。走遠路一次掉得多，這就是「走越遠士氣越低」。 */
export const MORALE_COST_PER_STEP = 0.04;
